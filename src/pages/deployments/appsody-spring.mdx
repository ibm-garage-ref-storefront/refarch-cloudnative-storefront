---
title: Deploy Storefront (Spring Version) locally on your machine
...
For instance, `<docker_host>` will be `host.docker.internal`.
...
- Clone the storefront-mobile repository
    ```
...
- To enable push notification on Android remember to replace `google-services.json` into the directory `ionic`
  ...
  All other required configurations are already preset into `mfpconfig/app_config.json` file.  Save `mfpconfig/app_config.json`
- Start Community Edition of IBM Mobile Foundation Services locally.  NOTE : This edition is free for trial and development purposes with the constraint that data will not be persisted beyond the lifetime of the services.
    ```
    ...
    ```
    You can verify if the services as follows: -
    ```
    docker ps
    CONTAINER ID        IMAGE                                   COMMAND                  CREATED           STATUS          PORTS                                NAMES
    ...
    ```
- Edit `ionic/config.xml` file and update the element <mfp:server runtime="mfp" url="http://localhostt:9080" /> for the url endpoint of the Mobile Foundation server
